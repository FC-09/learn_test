...
private:
    FileServer* server_;
    SOCKET client_;                    //客户端
    char recv_text_[BUF_SIZE];           //接收指令  recv_text[0]存放指令   recv_text_[1]---recv_text_[100]存放路径   recv_text_[102]---recv_text_[201]存放文件名
    char send_text_[BUF_SIZE];           //发送错误码及文件给客户端   
